...
const platform_client_1 = require("../src/generated/platform-client");
const tenant_client_1 = require("../src/generated/tenant-client");
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const child_process_1 = require("child_process");
const path_1 = __importDefault(require("path"));
const SALT_ROUNDS = 12;
async function main() {
    const platformDb = new platform_client_1.PrismaClient();
    try {
        console.log('🌊 ISSA Seed: Starting...\n');
        // ─── 1. Super Admin ───────────────────────────────────────
        console.log('→ Creating Super Admin...');
        const superAdminPassword = await bcryptjs_1.default.hash('SuperAdmin@123', SALT_ROUNDS);
        ...
        await platformDb.$executeRawUnsafe(`CREATE SCHEMA IF NOT EXISTS "${tenant.schemaName}"`);
        ...
        const tenantSchemaPath = path_1.default.resolve(process.cwd(), 'prisma', 'tenant-schema.prisma');
        ...
        (0, child_process_1.execSync)(`node --max-old-space-size=4096 "${prismaCliPath}" migrate deploy --schema="${tenantSchemaPath}"`, {
            env: {
                ...process.env,
        ...
        const tenantDb = new tenant_client_1.PrismaClient();
        // Use a transaction with SET LOCAL to target the tenant schema
        await tenantDb.$transaction(async (tx) => {
            await tx.$executeRawUnsafe(`SET LOCAL search_path = "${tenant.schemaName}"`);
            // Seed Branch
      ...
      `);
            ...
            const adminPasswordHash = await bcryptjs_1.default.hash('Admin@123', SALT_ROUNDS);
            await tx.$executeRawUnsafe(`
      ...
      `);
            ...
            const captainPasswordHash = await bcryptjs_1.default.hash('Captain@123', SALT_ROUNDS);
